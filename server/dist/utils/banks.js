"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.sortedBanks = exports.banks = void 0;
exports.getBankCodeByName = getBankCodeByName;
exports.banks = [
    {
        "name": "BANC CORP MICROFINANCE BANK",
        "routingKey": "090581",
        "bankCode": "090581",
        "categoryId": "9"
    },
    {
        "name": "STERLING BANK",
        "routingKey": "000001",
        "bankCode": "000001",
        "categoryId": "2"
    },
    {
        "name": "KEYSTONE BANK",
        "routingKey": "000002",
        "bankCode": "000002",
        "categoryId": "2"
    },
    {
        "name": "FIRST CITY MONUMENT BANK",
        "routingKey": "000003",
        "bankCode": "000003",
        "categoryId": "2"
    },
    {
        "name": "UNITED BANK FOR AFRICA",
        "routingKey": "000004",
        "bankCode": "000004",
        "categoryId": "2"
    },
    {
        "name": "JAIZ BANK",
        "routingKey": "000006",
        "bankCode": "000006",
        "categoryId": "2"
    },
    {
        "name": "FIDELITY BANK",
        "routingKey": "000007",
        "bankCode": "000007",
        "categoryId": "2"
    },
    {
        "name": "POLARIS BANK",
        "routingKey": "000008",
        "bankCode": "000008",
        "categoryId": "2"
    },
    {
        "name": "CITI BANK",
        "routingKey": "000009",
        "bankCode": "000009",
        "categoryId": "2"
    },
    {
        "name": "ECOBANK",
        "routingKey": "000010",
        "bankCode": "000010",
        "categoryId": "2"
    },
    {
        "name": "UNITY BANK",
        "routingKey": "000011",
        "bankCode": "000011",
        "categoryId": "2"
    },
    {
        "name": "STANBIC IBTC BANK",
        "routingKey": "000012",
        "bankCode": "000012",
        "categoryId": "2"
    },
    {
        "name": "GTBANK PLC",
        "routingKey": "000013",
        "bankCode": "000013",
        "categoryId": "2"
    },
    {
        "name": "ACCESS BANK",
        "routingKey": "000014",
        "bankCode": "000014",
        "categoryId": "2"
    },
    {
        "name": "ZENITH BANK",
        "routingKey": "000015",
        "bankCode": "000015",
        "categoryId": "2"
    },
    {
        "name": "FIRST BANK OF NIGERIA",
        "routingKey": "000016",
        "bankCode": "000016",
        "categoryId": "2"
    },
    {
        "name": "WEMA BANK",
        "routingKey": "000017",
        "bankCode": "000017",
        "categoryId": "2"
    },
    {
        "name": "UNION BANK",
        "routingKey": "000018",
        "bankCode": "000018",
        "categoryId": "2"
    },
    {
        "name": "ENTERPRISE BANK",
        "routingKey": "000019",
        "bankCode": "000019",
        "categoryId": "2"
    },
    {
        "name": "STANDARD CHARTERED BANK",
        "routingKey": "000021",
        "bankCode": "000021",
        "categoryId": "2"
    },
    {
        "name": "SUNTRUST BANK",
        "routingKey": "000022",
        "bankCode": "000022",
        "categoryId": "2"
    },
    {
        "name": "PROVIDUS BANK",
        "routingKey": "000023",
        "bankCode": "000023",
        "categoryId": "2"
    },
    {
        "name": "HERITAGE BANK",
        "routingKey": "000020",
        "bankCode": "000020",
        "categoryId": "2"
    },
    {
        "name": "ACCESS(DIAMOND) BANK",
        "routingKey": "000005",
        "bankCode": "000005",
        "categoryId": "2"
    },
    {
        "name": "GLOBUS BANK",
        "routingKey": "000027",
        "bankCode": "000027",
        "categoryId": "2"
    },
    {
        "name": "TITAN TRUST BANK",
        "routingKey": "000025",
        "bankCode": "000025",
        "categoryId": "2"
    },
    {
        "name": "PREMIUM TRUST BANK",
        "routingKey": "000031",
        "bankCode": "000031",
        "categoryId": "2"
    },
    {
        "name": "LOTUS BANK",
        "routingKey": "000029",
        "bankCode": "000029",
        "categoryId": "2"
    },
    {
        "name": "PARALLEX BANK",
        "routingKey": "000030",
        "bankCode": "000030",
        "categoryId": "2"
    },
    {
        "name": "TAJ BANK",
        "routingKey": "000026",
        "bankCode": "000026",
        "categoryId": "2"
    },
    {
        "name": "RAND MERCHANT BANK",
        "routingKey": "000024",
        "bankCode": "000024",
        "categoryId": "6"
    },
    {
        "name": "GREENWICH MERCHANT BANK",
        "routingKey": "060004",
        "bankCode": "060004",
        "categoryId": "6"
    },
    {
        "name": "NOVA MERCHANT BANK",
        "routingKey": "060003",
        "bankCode": "060003",
        "categoryId": "6"
    },
    {
        "name": "CORONATION MERCHANT BANK",
        "routingKey": "060001",
        "bankCode": "060001",
        "categoryId": "6"
    },
    {
        "name": "FBNQUEST MERCHANT BANK",
        "routingKey": "060002",
        "bankCode": "060002",
        "categoryId": "6"
    },
    {
        "name": "KUDA MICROFINANCE BANK",
        "routingKey": "090267",
        "bankCode": "090267",
        "categoryId": "9"
    },
    {
        "name": "SAFE HAVEN MICROFINANCE BANK",
        "routingKey": "090286",
        "bankCode": "090286",
        "categoryId": "9"
    },
    {
        "name": "PALMPAY",
        "routingKey": "100033",
        "bankCode": "100033",
        "categoryId": "10"
    },
    {
        "name": "OPAY",
        "routingKey": "100004",
        "bankCode": "100004",
        "categoryId": "12"
    },
    {
        "name": "9 PAYMENT SOLUTIONS BANK",
        "routingKey": "120001",
        "bankCode": "120001",
        "categoryId": "12"
    },
    {
        "name": "MOMO PAYMENT SERVICE BANK",
        "routingKey": "120003",
        "bankCode": "120003",
        "categoryId": "12"
    },
    {
        "name": "SMARTCASH PAYMENT SERVICE BANK",
        "routingKey": "120004",
        "bankCode": "120004",
        "categoryId": "0"
    },
    {
        "name": "PAGA",
        "routingKey": "100002",
        "bankCode": "100002",
        "categoryId": "10"
    },
    {
        "name": "FIRSTMONIE WALLET",
        "routingKey": "100014",
        "bankCode": "100014",
        "categoryId": "10"
    },
    {
        "name": "EARTHOLEUM",
        "routingKey": "100021",
        "bankCode": "100021",
        "categoryId": "10"
    },
    {
        "name": "VT NETWORKS",
        "routingKey": "100012",
        "bankCode": "100012",
        "categoryId": "10"
    },
    {
        "name": "KONGAPAY",
        "routingKey": "100025",
        "bankCode": "100025",
        "categoryId": "10"
    },
    {
        "name": "EKONDO MICROFINANCE BANK",
        "routingKey": "090097",
        "bankCode": "090097",
        "categoryId": "7"
    },
    {
        "name": "FSDH",
        "routingKey": "400001",
        "bankCode": "400001",
        "categoryId": "4"
    },
    {
        "name": "SAFE HAVEN SANDBOX BANK",
        "routingKey": "999240",
        "bankCode": "999240",
        "categoryId": "9"
    }
];
// Sort banks alphabetically
exports.sortedBanks = exports.banks.sort((a, b) => a.name.localeCompare(b.name));
// Helper function to get bank code by name
function getBankCodeByName(bankName) {
    const bank = exports.banks.find(b => b.name.toLowerCase() === bankName.toLowerCase());
    return bank ? bank.bankCode : null;
}
//# sourceMappingURL=banks.js.map