...
router.post('/send', descopeAuth_1.devAuth, async (req, res) => {
    try {
        ...
        // Send OTP email
        const emailSent = await emailService_1.emailService.sendOTPEmail(userEmail, otpCode);
        if (!emailSent) {
            logger_1.logger.error(`Failed to send OTP email to user: ${userId}`);
            return res.status(500).json({ success: false, error: 'Failed to send OTP email' });
        }
        logger_1.logger.info(`OTP sent to user: ${userId}, email: ${userEmail}`);
        res.json({
            success: true,
        ...
        logger_1.logger.error('OTP send error:', error);
...
router.post('/verify', descopeAuth_1.devAuth, async (req, res) => {
    try {
        ...
        logger_1.logger.info(`OTP verified successfully for user: ${userId}`);
        res.json({
            success: true,
        ...
        logger_1.logger.error('OTP verify error:', error);
...
router.post('/resend', descopeAuth_1.devAuth, async (req, res) => {
    try {
        ...
        const emailSent = await emailService_1.emailService.sendOTPEmail(userEmail, otpCode);
        if (!emailSent) {
            logger_1.logger.error(`Failed to resend OTP email to user: ${userId}`);
            return res.status(500).json({ success: false, error: 'Failed to send OTP email' });
        }
        logger_1.logger.info(`OTP resent to user: ${userId}, email: ${userEmail}`);
        res.json({
            success: true,
        ...
        logger_1.logger.error('OTP resend error:', error);
