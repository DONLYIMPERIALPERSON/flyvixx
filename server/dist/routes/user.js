...
router.get('/profile', descopeAuth_1.devAuth, async (req, res) => {
    try {
        ...
        logger_1.logger.info('Profile request for user:', userId);
...
router.put('/profile', descopeAuth_1.devAuth, async (req, res) => {
    try {
        ...
        logger_1.logger.info('Profile update request for user:', userId);
        ...
        if (payoutDetails !== undefined) {
            logger_1.logger.info('Updating payout details:', JSON.stringify(payoutDetails, null, 2));
            // Validate payout details structure
            const validationError = validatePayoutDetails(payoutDetails);
            if (validationError) {
                logger_1.logger.error('Payout details validation error:', validationError);
                return res.status(400).json({ success: false, error: validationError });
            }
            user.payoutDetails = payoutDetails;
            user.updatedAt = new Date();
            logger_1.logger.info('Payout details updated successfully');
        }
...
router.get('/balance', descopeAuth_1.devAuth, async (req, res) => {
    try {
        ...
        logger_1.logger.info('Balance request for user:', userId);
        const userRepository = database_1.AppDataSource.getRepository(User_1.User);
        const user = await userRepository.findOne({
            where: { id: userId },
