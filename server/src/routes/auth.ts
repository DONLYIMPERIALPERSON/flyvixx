...
import { validateDescopeToken, AuthenticatedRequest } from '../middleware/descopeAuth';
...
router.post('/verify', validateDescopeToken, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    logger.info(`Token verification for user: ${descopeUser.sub}`);
    ...
    if (!user) {
      ...
      if (existingUserByEmail) {
        ...
        logger.info(`Merging user account: updating ID from ${existingUserByEmail.id} to ${descopeUser.sub} for email ${descopeUser.email}`);

        // Update the existing user with the new Descope ID using raw SQL to avoid unique constraint issues
        await userRepository.query(
          `UPDATE users SET id = $1, "lastLoginAt" = $2, "emailVerified" = $3 WHERE id = $4`,
          [descopeUser.sub, new Date(), descopeUser.email_verified || existingUserByEmail.emailVerified, existingUserByEmail.id]
        );
        ...
        logger.info(`Merged user account for: ${user.email} (new ID: ${user.id})`);
      } else {
        ...
        isNewUser = true;
        logger.info(`Created new user in database: ${user.id}`);
      }
    } else {
      ...
      await userRepository.save(user);
      logger.info(`Updated last login for user: ${user.id}`);
    }

    // If this is a new user and they have a referral code, process the referral
    if (isNewUser && referralCode) {
      logger.info(`🔄 Processing referral code ${referralCode} for new user ${user.id} (${user.email})`);
      ...
      try {
        ...
        if (referrer) {
          logger.info(`✅ Found referrer: ${referrer.id} (${referrer.email}) for referral code ${referralCode}`);
          ...
          try {
            const emailSent = await emailService.sendReferralNotificationEmail(referrer.email, user.email);
            if (emailSent) {
              logger.info(`📧 Referral notification email sent to ${referrer.email} for new user ${user.email}`);
            } else {
              logger.error(`❌ Failed to send referral notification email to ${referrer.email}`);
            }
          } catch (emailError) {
            logger.error('❌ Failed to send referral notification email:', emailError);
          }

          // Create referral notification for the referrer
          try {
            await NotificationService.createReferralNotification(referrer.id, user.email);
            logger.info(`🔔 Referral notification created for user ${referrer.id} about new user ${user.email}`);
          } catch (notificationError) {
            logger.error('❌ Failed to create referral notification:', notificationError);
          }

          logger.info(`🎉 User ${user.id} successfully joined with referral code ${referralCode} from user ${referrer.id}`);
        } else {
          logger.warn(`⚠️ Invalid referral code ${referralCode} provided by user ${user.id} - no referrer found`);
        }
      } catch (referralError) {
      ...
      logger.info(`ℹ️ New user ${user.id} signed up without referral code`);
    } else if (referralCode) {
      logger.info(`ℹ️ Existing user ${user.id} provided referral code ${referralCode} (ignored)`);
    }
...
router.post('/login', async (req, res) => {
  try {
    ...
    logger.warn(`Legacy login attempt for: ${email} - should use Descope`);
...
router.post('/register', async (req, res) => {
  try {
    ...
    logger.warn(`Legacy registration attempt for: ${email} - should use Descope`);
...
router.post('/logout', validateDescopeToken, async (req: AuthenticatedRequest, res) => {
  try {
