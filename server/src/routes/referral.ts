...
router.get('/info', validateDescopeToken, async (req, res) => {
  try {
    ...
    if (!referralCode) {
      referralCode = `FLY${uuidv4().substring(0, 8).toUpperCase()}`;
      await userRepository.update(userId, { referralCode });
      logger.info(`Generated referral code for user: ${userId}, code: ${referralCode}`);
    }
...
router.post('/join', async (req, res) => {
  try {
    ...
    logger.info(`User ${userId} joined with referral code ${referralCode} from user ${referrer.id}`);

    // Send referral notification email to the referrer
    try {
      await emailService.sendReferralNotificationEmail(referrer.email, newUser.email);
      logger.info(`Referral notification email sent to ${referrer.email} for new user ${newUser.email}`);
    } catch (emailError) {
    ...
    try {
      await NotificationService.createReferralNotification(referrer.id, newUser.email);
      logger.info(`Referral notification created for user ${referrer.id} about new user ${newUser.email}`);
    ...
    logger.error('Referral join error:', error);
...
router.get('/stats', validateDescopeToken, async (req, res) => {
  try {
    const userId = (req as any).user?.sub;
    if (!userId) {
      return res.status(401).json({ success: false, error: 'Unauthorized' });
