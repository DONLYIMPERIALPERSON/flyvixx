 ...
 */
router.get('/token', validateDescopeToken, async (req, res) => {
    try {
        const accessToken = await safeHavenAuth.getAccessToken();
 ...
 */
router.post('/verify-bank', validateDescopeToken, async (req, res) => {
    try {
        ...
        const result = await safeHavenAuth.verifyBankAccount(bankCode, accountNumber);
 ...
 */
router.post('/virtual-account', validateDescopeToken, async (req, res) => {
    try {
        ...
        logger.info(`Deposit conversion: $${amount} USD = ₦${amountNGN} NGN (rate: ${EXCHANGE_RATE})`);

        // Generate unique external reference
        const externalReference = `deposit_${userId}_${Date.now()}`;

        const result = await safeHavenAuth.createVirtualAccount(amountNGN, externalReference);
...
router.post('/transfer', validateDescopeToken, async (req, res) => {
    try {
        ...
        logger.info(`Withdrawal request received: userId=${userId}, amount=${amount}, method=${method}`);
        ...
        if (!method) {
            logger.error('No method provided');
            return res.status(400).json({
                success: false,
        ...
        logger.info(`Withdrawal check - user payout details: ${JSON.stringify(user.payoutDetails)}`);

        if (!user.payoutDetails?.bank?.accountName) {
            logger.error('Missing accountName in payout details');
            return res.status(400).json({
                success: false,
        ...
        if (!user.payoutDetails?.bank?.accountNumber) {
            logger.error('Missing accountNumber in payout details');
            return res.status(400).json({
                success: false,
        ...
        if (!user.payoutDetails?.bank?.bankName) {
            logger.error('Missing bankName in payout details');
            return res.status(400).json({
                success: false,
        ...
        logger.info(`Payout details validation passed: accountName=${user.payoutDetails.bank.accountName}, accountNumber=${user.payoutDetails.bank.accountNumber}, bankName=${user.payoutDetails.bank.bankName}`);
        ...
        if (!beneficiaryBankCode) {
            logger.error(`Bank name not found in bank codes mapping: ${bankDetails.bankName}`);
            return res.status(400).json({
                success: false,
        ...
        try {
            logger.info(`Performing fresh name enquiry for withdrawal: bankCode=${beneficiaryBankCode}, accountNumber=${beneficiaryAccountNumber}`);
            ...
            if (nameEnquiryResult?.data?.sessionId) {
                ...
                logger.info(`Fresh name enquiry successful, sessionId: ${freshSessionId}`);
            } else {
                logger.error('Fresh name enquiry failed - no sessionId returned:', nameEnquiryResult);
                return res.status(500).json({
                    success: false,
            ...
            logger.error('Fresh name enquiry error:', error);
            return res.status(500).json({
                success: false,
                error: 'Failed to verify beneficiary account details'
            });
        ...
        if (requiresApproval) {
            ...
            logger.info(`Large withdrawal pending approval: User ${userId}, Amount $${amount}, Transaction ${transaction.id}`);
            ...
            logger.info(`Initiating SafeHaven transfer: ${JSON.stringify(transferData)}`);
            ...
            try {
                ...
                logger.info(`SafeHaven transfer initiated: ${JSON.stringify(transferResult, null, 2)}`);
                ...
                logger.error('SafeHaven transfer error (but transaction marked as completed):', error);
            ...
            logger.info(`Withdrawal completed successfully: User ${userId}, Amount $${amount}, Transaction ${transaction.id}`);

            // Create withdrawal notification
            try {
                const notification = await NotificationService.createWithdrawalNotification(userId, amount);
                logger.info(`✅ Withdrawal notification created for user ${userId}: ${notification.id}`);
            ...
            try {
                const emailSent = await emailService.sendWithdrawalEmail(user.email, amount.toFixed(2));
                if (emailSent) {
                    logger.info(`Withdrawal notification email sent to ${user.email}`);
                } else {
                    logger.warn(`Withdrawal notification email failed to send to ${user.email} - service returned false`);
                }
            } catch (emailError) {
                logger.error('Failed to send withdrawal notification email:', emailError);
        ...
        logger.error('SafeHaven transfer error:', error);
        res.status(500).json({
            success: false,
...
router.post('/webhook', async (req, res) => {
    try {
        ...
        logger.info(`SafeHaven webhook received: ${JSON.stringify(webhookData)}`);
        ...
        if (status !== 'successful' && status !== 'success' && status !== 'Completed' && status !== 'completed') {
            logger.info(`Ignoring non-successful transaction: ${status}`);
        ...
        logger.info(`Webhook amount processing: amount=${amount}, receivedAmountNGN=${receivedAmountNGN}`);

        // Convert NGN to USD using our exchange rate (1500 NGN per USD)
        const originalAmount = receivedAmountNGN / 1500;

        logger.info(`Amount conversion: receivedAmountNGN=${receivedAmountNGN} / 1500 = originalAmount=${originalAmount}`);
        ...
        if (existingTransaction) {
            logger.info(`Duplicate transaction detected: ${paymentReference}`);
        ...
        await AppDataSource.transaction(async (transactionalEntityManager) => {
            ...
            try {
                ...
                logger.info(`✅ Deposit notification created for user ${userId}: ${notification.id}`);
            } catch (notificationError) {
                logger.error('❌ Failed to create deposit notification:', notificationError);
                // Don't fail the webhook if notification creation fails
            }

            // Send deposit notification email
            try {
                await emailService.sendDepositEmail(user.email, originalAmount.toFixed(2));
                logger.info(`Deposit notification email sent to ${user.email}`);
            } catch (emailError) {
                logger.error('Failed to send deposit notification email:', emailError);
        ...
        logger.error('SafeHaven webhook processing error:', error);
        res.status(500).json({
            success: false,
 ...
 */
router.post('/admin/approve-withdrawal', validateDescopeToken, async (req, res) => {
    try {
        const userId = (req as any).user?.sub;
        ...
        logger.info(`Admin approving withdrawal: ${JSON.stringify(transferData)}`);
        ...
        try {
            ...
            if (isTransferSuccessful) {
                ...
                try {
                    const emailSent = await emailService.sendWithdrawalEmail(withdrawalUser.email, transaction.amount.toFixed(2));
                    if (emailSent) {
                        logger.info(`Withdrawal notification email sent to ${withdrawalUser.email} for approved withdrawal`);
                    } else {
                        logger.warn(`Withdrawal notification email failed to send to ${withdrawalUser.email} for approved withdrawal - service returned false`);
                    }
                } catch (emailError) {
                    logger.error('Failed to send withdrawal notification email for approved withdrawal:', emailError);
                ...
                logger.error(`Approved withdrawal failed: Transaction ${transaction.id}`);
            ...
            logger.error(`Approved withdrawal error: Transaction ${transaction.id}`, error);
 ...
 */
router.post('/admin/retry-withdrawal', validateDescopeToken, async (req, res) => {
    try {
        const userId = (req as any).user?.sub;
        const { transactionId } = req.body;
        ...
        if (transaction.status !== TransactionStatus.FAILED || !transaction.metadata?.adminRetryAvailable) {
            return res.status(400).json({
                success: false,
        ...
        logger.info(`Admin retrying withdrawal: ${JSON.stringify(transferData)}`);
        ...
        try {
            ...
            if (isTransferSuccessful) {
                ...
                logger.error(`Withdrawal retry failed: Transaction ${transaction.id}`);
            ...
            logger.error(`Withdrawal retry error: Transaction ${transaction.id}`, error);
